...
class UsersRepository extends BaseRepository
{
	...
	public function disable(User $instance)
	{
		...
		if (auth()->user())
			$instance->disabled_by_user_id = auth()->user()->id;
		$instance->save();
		return $instance;
	}
	...
	public function addFreeTrial($user)
	{
		$user_count = User::where('profile_setup_step',User::LOGGED_IN_PAGE)->count();

		if ($user_count <= config('app.freeUsersLimit')) {
			$user->is_free_trial = 1;
